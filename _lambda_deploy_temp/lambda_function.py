...
def get_today_picks(headers):
    ...
    # Sort by score before applying guardrails and final top-5 composition.
    ...
    # Build final slate: max top 5 with preferred composition of
    # 2 long-odds picks (>4/1) and 3 standard picks.
    ...
    final_picks.extend(standard_odds[:3])

    # Fill remaining slots up to five from the strongest leftovers.
    if len(final_picks) < 5:
        selected_ids = {str(p.get('bet_id', '')) for p in final_picks}
        leftovers = [
            pick for pick in future_picks
            if str(pick.get('bet_id', '')) not in selected_ids
        ]
        final_picks.extend(leftovers[: max(0, 5 - len(final_picks))])

    future_picks = final_picks[:5]
    # Rank and sort by race time for display.
    future_picks.sort(key=lambda x: x.get('race_time', ''))
    for idx, pick in enumerate(future_picks, start=1):
        pick['pick_rank'] = idx
