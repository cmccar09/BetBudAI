...
# ---------------------------------------------------------------------------
_CACHE_DIR = '/tmp' if os.environ.get('AWS_LAMBDA_FUNCTION_NAME') else '.'
CACHE_FILE = os.path.join(_CACHE_DIR, 'form_cache.json')
CACHE_TTL_HOURS = 12
...
# ---------------------------------------------------------------------------
_SL_ID_FILE = os.path.join(_CACHE_DIR, '_sl_horse_ids.json')
...
def _fetch_sl_race_form(race_url: str) -> dict:
    ...
    for ride in rides:
        ...
        # Extract Timeform star rating (1-5)
...
def enrich_runners(races: list[dict], verbose: bool = True) -> list[dict]:
    ...
    for race in races:
        ...
        for race_url in sl_urls:
            race_form, tf_stars_map = _fetch_sl_race_form(race_url)
            _today_form.update(race_form)
            _today_tf_stars.update(tf_stars_map)
            time.sleep(0.4)
    ...
    # Step 3: Inject form_runs + timeform_stars into each runner
    ...
    for race in races:
        for runner in race.get('runners', []):
            ...
            if name:
                ...
                # Inject Timeform star rating if available
                ...
                if verbose:
                    ...
                    print(f"  [{done}/{total_horses}] {name}: {status}{tf_tag}")

    if verbose:
        print(f"  [form] Enriched {enriched}/{done} runners with form data")
        if tf_count:
            print(f"  [form] Timeform stars injected for {tf_count}/{done} runners")
    return races
...
def get_form_signals(horse_data: dict, today_course: str, today_distance_f: float | None,
    ...
    signals = {
        ...
        'form_velocity': 0.0,           # position trend (positive = improving)
        'form_velocity_improving': False,  # improving by 2+ places avg
        'form_velocity_declining': False,  # declining by 2+ places avg
    }
    ...
    # Form velocity — finishing position trend over last 4 runs (lower = better)
    # Positive velocity = improving (positions trending down), negative = declining
    valid_positions = [r.get('position') for r in runs[:4] if r.get('position') and isinstance(r.get('position'), int) and r['position'] <= 20]
    if len(valid_positions) >= 3:
        # runs[0] is most recent. Compute weighted improvement: recent vs older
        # slope: negative means finishing positions getting smaller (= improving)
        n = len(valid_positions)
        # Simple linear: compare avg of first half (recent) to avg of second half (older)
        recent_avg = sum(valid_positions[:n//2]) / max(1, n//2)
        older_avg = sum(valid_positions[n//2:]) / max(1, n - n//2)
        velocity = older_avg - recent_avg  # positive = improving (was 6th, now 2nd)
        signals['form_velocity'] = round(velocity, 2)
        signals['form_velocity_improving'] = velocity >= 2.0   # Improving by 2+ places on avg
        signals['form_velocity_declining'] = velocity <= -2.0   # Declining by 2+ places
    else:
        signals['form_velocity'] = 0.0
        signals['form_velocity_improving'] = False
        signals['form_velocity_declining'] = False
