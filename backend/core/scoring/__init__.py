...
# weather_going_inference — try package-relative first, then flat (Lambda bundle)
try:
...
# track_daily_insights — try package-relative first, then flat (Lambda bundle)
try:
    from ...utils.track_daily_insights import get_track_insights
except ImportError:
    try:
        from backend.utils.track_daily_insights import get_track_insights
    except ImportError:
        try:
...
try:
    from ..enrichment.form_enricher import get_form_signals, _dist_to_furlongs
    FORM_ENRICHER_AVAILABLE = True
except ImportError:
    try:
        from backend.core.enrichment.form_enricher import get_form_signals, _dist_to_furlongs
        FORM_ENRICHER_AVAILABLE = True
    except ImportError:
        try:
...
try:
    ...
    try:
        from backend.core.scoring.trainer_form_stats import hot_form_bonus as _hot_form_bonus
        TRAINER_FORM_AVAILABLE = True
    except ImportError:
        try:
            from trainer_form_stats import hot_form_bonus as _hot_form_bonus
...
# Import Cheltenham analyzer when at Cheltenham (optional — festival-only module)
try:
...
# ─── PHASE 1 FREE SIGNALS (2026-05-20) ────────────────────────────────────────
# Run Style, Jockey Upgrade (immediate), Equipment/Liquidity (pending data extraction)
try:
    from ..signals import (
        classify_run_style,
        predict_race_pace,
    ...
    print("[PHASE1] Signals loaded: Run Style + Jockey Upgrade active")
except ImportError:
    try:
        # Lambda flat structure fallback
        from signals import (
            classify_run_style,
            predict_race_pace,
            calculate_pace_match_bonus,
        ...
        PHASE1_SIGNALS_AVAILABLE = True
        print("[PHASE1] Signals loaded (Lambda flat structure): Run Style + Jockey Upgrade active")
    except ImportError as e:
        PHASE1_SIGNALS_AVAILABLE = False
        print(f"[PHASE1] Signals not available: {e}")
        def classify_run_style(*a, **kw): return 'UNKNOWN'
        def predict_race_pace(*a, **kw): return 'STEADY'
        def calculate_pace_match_bonus(*a, **kw): return 0, ''
        def detect_jockey_upgrade(*a, **kw): return 0, ''
        def detect_jockey_downgrade(*a, **kw): return 0, ''
...
DEFAULT_WEIGHTS = {
    'sweet_spot':           10,  # REDUCED 12->10 (2026-04-27): odds bands were still over-rewarding losing selections
    'optimal_odds':          8,  # REDUCED 10->8 (2026-04-27): market-position signal should help, not dominate total score
    ...
    'consistency':           6,  # RAISED 4->6 (2026-04-27): recent loss audit showed winners beat us on consistency most often
    ...
    'trainer_reputation':   16,  # RAISED 15->16 (2026-04-27): winners repeatedly had trainer edge we under-scored
    'trainer_tier2':         8,  # unchanged: good trainers
    'trainer_tier3':         4,  # unchanged: decent trainers
    'favorite_correction':   8,  # RAISED 7->8 (2026-04-27): recent winners kept showing stronger market backing than our losing picks
    ...
    'distance_suitability': 16,  # REDUCED 18->16 (2026-04-27): helping score inflation on horses that still lose badly
    'novice_race_penalty':  15,  # unchanged
    'bounce_back_bonus':     8,  # REDUCED 12->8
    'pu_winner_bounce':      6,  # NEW 2026-04-20: proven winner (1 in form) + P/U/F history + last run 1/2/3
    ...
    'cd_bonus':             16,  # REDUCED 18->16 (2026-04-27): keep useful, but avoid inflating top-ranked losers
    ...
    # Form velocity (2026-04-20) — Lesson from HorseRacingOracle guide: linear trend on finishing
    # positions over last 3-4 runs detects improving horses that static form misses.
    'form_velocity_bonus':    10,  # Horse improving by 2+ places avg over last 4 runs
    'form_velocity_penalty':   6,  # Horse declining by 2+ places avg — confidence drop
    # Trainer-course familiarity (2026-04-20) — Mirror of jockey_course_bonus.
    # Some trainers have massive track biases (Mullins at Punchestown, Elliott at Fairyhouse).
    'trainer_course_bonus':    8,  # Trainer has ≥20% WR at this specific course (min 3 runs)
    # Market steam detection (2026-04-20) — price_movement from betfair_odds_fetcher.
    # Already written to runner as 'steaming' or 'drifting'. Now scored.
    'market_steam_bonus':     10,  # Price shortened ≥20% since first fetch (money coming in)
    'market_drift_penalty':    6,  # Price drifted ≥25% (market losing confidence)
    # Risk controls from 2026-04-24 loss review
    'recent_non_completion_penalty': 10,  # Last-2 runs include P/U/F/R -> reliability downgrade
    'market_divergence_penalty':     14,  # Model top pick materially weaker than market favourite profile
    'score_gap_illusion_penalty':    12,  # Huge model gap but near-equal market odds (overconfidence guard)
    'current_form_edge_bonus':        8,   # Reward horses with real current form, not just theoretical upside
    'potential_hype_penalty':         10,  # Penalise upside/projection when current form is weak or absent
    # Timeform star rating (2026-04-20) — scraped free from Sporting Life racecards.
    # 5 stars = top-rated, 1 star = Timeform sees it as poor. Extremely strong signal.
    'timeform_5star_bonus':   12,  # Top-rated by Timeform
    'timeform_4star_bonus':    8,  # Good Timeform rating
    'timeform_3star_bonus':    4,  # Average Timeform view
    'timeform_lowstar_penalty': 6,  # 1-2 stars — Timeform thinks poor chance
    ...
    'class_drop_rebound_bonus':   10,  # Poor recent form + clear class drop can rebound in weaker grade
    # Trainer combo bonus (2026-04-24) — LESSON: 4/21 fortnight losses had winner beat us on
    # trainer_reputation alone; 2/21 had trainer_hot_form alone. When BOTH fire simultaneously
    # (elite/good trainer who is also currently in hot form), that confluence is stronger than
    # either signal alone — trainer confidence at its peak.
    'trainer_combo_bonus':        8,  # trainer_reputation>0 AND trainer_hot_form>0 → extra +8
...
def analyze_horse_comprehensive(horse_data, course, avg_winner_odds=3.80, course_winners_today=0, field_weights=None, meeting_context=None, n_runners=0):
    ...
    breakdown = {}
    reasons = []
    _form_velocity_improving = False
    ...
    sweet_spot_pts = 0
    # REBALANCED 2026-04-27: settled UI picks since 2026-03-01 show
    # <3 odds best, 3-5 acceptable, 5-8 worst-performing live band.
    # Reward shorter prices, keep 3-5 modest, and sharply reduce 5-8 support.
    if 2.0 <= odds < 3.0:
        # Best-performing live band.
        sweet_spot_pts = int(weights['sweet_spot'] * 0.85)
        reasons.append(f"Strong odds range (2-3, highest SR): {sweet_spot_pts}pts")
    elif 3.0 <= odds < 5.0:
        # Weak live band (17% SR) — ATOMIC FIX #3: reduce from 0.35 to 0.15 to starve this band
        sweet_spot_pts = int(weights['sweet_spot'] * 0.15)
        reasons.append(f"Mid-range odds (3-5, empirically weak): {sweet_spot_pts}pts")
    elif 5.0 <= odds <= 8.0:
        # Worst-performing live UI band; keep near-neutral unless the horse is exceptional elsewhere.
        sweet_spot_pts = int(weights['sweet_spot'] * 0.1)
        reasons.append(f"Cautious odds range (5-8, weak live SR): {sweet_spot_pts}pts")
    elif 8.0 < odds <= 9.0:
        # Broad value tail
        sweet_spot_pts = int(weights['sweet_spot'] * 0.3)
    ...
    # Consistency (places) — CAPPED at 3 places worth (2026-04-20)
    # LESSON: Delgany Deadline, Turndlightsdownlow, Bond Broker all got 24pts (4×6)
    # from consistency alone — none won. Consistent placers ≠ winners.
    place_pts_each = int(weights['consistency'])
    place_points = min(places, 3) * place_pts_each  # Cap at 3 places
    score += place_points
    breakdown['consistency'] = place_points
    if places > 0:
        reasons.append(f"{places} places (2nd/3rd): {place_points}pts" + (' (capped at 3)' if places > 3 else ''))
    ...
    # Detect patterns like 2-6-1 or 2-5-1 or 2-P-1 showing recovery after poor run
    # UPDATED 2026-04-20: Kilmore Rock (form 1-13P6PP3) won at 6/1 but scored 51.
    # P/U/F (pulled up, unseated, fell) are now treated as poor runs in bounce-back check.
    _BAD_RUNS = {'5', '6', '7', '8', '9', '0', 'P', 'U', 'F', 'R', 'B', 'D', 'S', 'L'}
    _GOOD_RUNS = {'1', '2', '3'}
    bounce_back_pts = int(weights.get('bounce_back_bonus', 12))
    if form:
        form_parts = form.replace('/', '-').split('-')
        # Filter out empty parts
        form_parts = [p.strip() for p in form_parts if p.strip()]
        if len(form_parts) >= 3:
            try:
                last_3 = form_parts[-3:]
                if (last_3[0] in _GOOD_RUNS and
                    last_3[1].upper() in _BAD_RUNS and
                    last_3[2] in _GOOD_RUNS):
    ...
    # 12b. PROVEN WINNER WITH PU FORM (NEW 2026-04-20)
    # LESSON: Kilmore Rock form=1-13P6PP3, 2 wins in profile but recent P's tanked the
    # deep_form score to +10. Won at 6/1. Horses with genuine winning ability (1's in form)
    # who also have P/U/F runs are inconsistent but dangerous — they have proven class.
    # When their most recent run is decent (1-3), they may be bouncing back.
    _pu_winner_bonus = int(weights.get('pu_winner_bounce', 6))
    if form:
        _form_chars = form.replace('-', '').replace('/', '').upper()
        _has_wins = '1' in _form_chars
        _has_pu = any(c in _form_chars for c in ('P', 'U', 'F'))
        _last_char = _form_chars[-1] if _form_chars else ''
        if _has_wins and _has_pu and _last_char in ('1', '2', '3'):
            # Proven ability + showed up last time despite PU history
            score += _pu_winner_bonus
            breakdown['pu_winner_bounce'] = _pu_winner_bonus
            reasons.append(f"📈 Proven winner with P/U form, last run {_last_char} (dangerous bounce): +{_pu_winner_bonus}pts")
        else:
            breakdown['pu_winner_bounce'] = 0
    else:
        breakdown['pu_winner_bounce'] = 0

    # 12c. RECENT NON-COMPLETION PENALTY (2026-04-24)
    # Lesson: horses with very recent P/U/F/R are currently under-penalised in headline score.
    # Apply a reliability haircut when a non-completion appears in the last 1-2 runs.
    _recent_nc_penalty = int(weights.get('recent_non_completion_penalty', 10))
    if form:
        _nc_chars = [c.upper() for c in form.replace('-', '').replace('/', '') if c.strip()]
        _recent_2 = _nc_chars[-2:] if len(_nc_chars) >= 2 else _nc_chars
        _nc_count = sum(1 for c in _recent_2 if c in ('P', 'U', 'F', 'R'))
        if _nc_count > 0:
            # Last-run non-completion is harsher than second-last.
            _last_nc = _recent_2[-1] in ('P', 'U', 'F', 'R') if _recent_2 else False
            _pen = _recent_nc_penalty if _last_nc else max(4, _recent_nc_penalty // 2)
            score -= _pen
            breakdown['recent_non_completion_penalty'] = -_pen
            reasons.append(f"Recent non-completion in form ({''.join(_recent_2)}): -{_pen}pts")
        else:
            breakdown['recent_non_completion_penalty'] = 0
    else:
        breakdown['recent_non_completion_penalty'] = 0
    ...
    # 14c2. TRAINER-COURSE FAMILIARITY (2026-04-20) — bonus if trainer wins here often
    tc_bonus_pts = int(weights.get('trainer_course_bonus', 8))
    trainer_for_course = str(horse_data.get('trainer', '')).strip()
    if trainer_for_course and course:
        try:
            tc_key = f"TRAINER_COURSE_{trainer_for_course.replace(' ', '_')}_{course.replace(' ', '_')}"
            _db_tc = boto3.resource('dynamodb', region_name='eu-west-1')
            _tbl_tc = _db_tc.Table('SureBetBets')
            tc_resp = _tbl_tc.get_item(Key={'bet_id': tc_key, 'bet_date': 'HISTORY'})
            tc_item = tc_resp.get('Item', {})
            tc_wins = int(tc_item.get('course_wins', 0))
            tc_runs = int(tc_item.get('course_runs', 0))
            if tc_runs >= 3 and tc_wins >= 1:
                tc_win_rate = tc_wins / tc_runs
                if tc_win_rate >= 0.20:
                    tc_pts = tc_bonus_pts
                elif tc_win_rate >= 0.12:
                    tc_pts = tc_bonus_pts // 2
                else:
                    tc_pts = 0
                if tc_pts > 0:
                    score += tc_pts
                    breakdown['trainer_course_bonus'] = tc_pts
                    reasons.append(f"Trainer {trainer_for_course} {tc_wins}/{tc_runs} wins at {course}: +{tc_pts}pts")
                else:
                    breakdown['trainer_course_bonus'] = 0
            else:
                breakdown['trainer_course_bonus'] = 0
        except Exception:
            breakdown['trainer_course_bonus'] = 0
    ...
    if FORM_ENRICHER_AVAILABLE and horse_data.get('form_runs'):
        ...
        if fs.get('class_drop'):
            ...
            # Poor-form rebound check (2026-04-24): some horses with ugly recent figures
            # can still win when falling sharply in class; reward that specific setup.
            _raw_recent = str(form or '').replace('-', '').replace('/', '').upper()
            _recent3 = _raw_recent[-3:] if _raw_recent else ''
            _poor_tokens = set('0456789PUFRBDSL')
            _poor_count = sum(1 for c in _recent3 if c in _poor_tokens)
            _has_recent_win = '1' in _recent3
            if _recent3 and _poor_count >= 2 and not _has_recent_win:
                rb_pts = int(weights.get('class_drop_rebound_bonus', 10))
                form_detail_pts += rb_pts
                reasons.append(f"Poor recent form ({_recent3}) but big class drop rebound angle: +{rb_pts}pts")

        # Form velocity — finishing position trend over last 3-4 runs
        if fs.get('form_velocity_improving'):
            pts = int(weights.get('form_velocity_bonus', 10))
            form_detail_pts += pts
            _form_velocity_improving = True
            reasons.append(f"📈 Form velocity +{fs['form_velocity']:.1f} (improving trend over last runs): +{pts}pts")
        elif fs.get('form_velocity_declining'):
            pts = int(weights.get('form_velocity_penalty', 6))
            form_detail_pts -= pts
            reasons.append(f"📉 Form velocity {fs['form_velocity']:.1f} (declining trend): -{pts}pts")
    ...
    # 13b. CURRENT FORM VS IMPROVED POTENTIAL (2026-04-24)
    # Force a distinction between horses that are already running well now versus horses
    # we only think might improve. Potential can support a pick, but should not dominate a
    # weak current-form profile unless other evidence is very strong.
    _form_digits = [int(c) for c in str(form).replace('-', '').replace('/', '') if c.isdigit()]
    _recent_digits = _form_digits[-3:] if _form_digits else []
    _current_form_hits = sum(1 for d in _recent_digits if d <= 3)
    _current_form_weak = bool(_recent_digits) and all(d >= 4 for d in _recent_digits)
    _potential_signals = sum([
        1 if breakdown.get('short_form_improvement', 0) > 0 else 0,
        1 if breakdown.get('unexposed_bonus', 0) > 0 else 0,
        1 if _form_velocity_improving else 0,
    ])
    _current_form_bonus = int(weights.get('current_form_edge_bonus', 8))
    _potential_hype_pen = int(weights.get('potential_hype_penalty', 10))

    if _current_form_hits >= 2 or recent_win:
        _bonus = _current_form_bonus if _current_form_hits >= 2 else max(3, _current_form_bonus // 2)
        score += _bonus
        breakdown['current_form_edge'] = _bonus
        reasons.append(f"Current form stronger than projection ({''.join(map(str, _recent_digits)) or form}): +{_bonus}pts")
    elif _potential_signals >= 2 and _current_form_weak and wins == 0:
        score -= _potential_hype_pen
        breakdown['potential_hype_penalty'] = -_potential_hype_pen
        reasons.append(
            f"Potential outruns current form ({''.join(map(str, _recent_digits)) or form}) — avoid projection-only pick: -{_potential_hype_pen}pts"
        )
    elif _potential_signals >= 1 and _current_form_hits == 0 and not recent_win and wins == 0:
        _pen = max(4, _potential_hype_pen // 2)
        score -= _pen
        breakdown['potential_hype_penalty'] = -_pen
        reasons.append(f"Improver angle lacks live current-form proof: -{_pen}pts")
    else:
        breakdown['current_form_edge'] = 0
        breakdown['potential_hype_penalty'] = 0
    ...
    # ── TRAINER COMBO BONUS (2026-04-24) ─────────────────────────────────────
    # When a reputable trainer is ALSO currently in hot form, their confidence is
    # at its peak — amplify both signals with a combo multiplier.
    if breakdown.get('trainer_reputation', 0) > 0 and breakdown.get('trainer_hot_form', 0) > 0:
        _combo_pts = int(weights.get('trainer_combo_bonus', 8))
        score += _combo_pts
        breakdown['trainer_combo'] = _combo_pts
        reasons.append(f"⭐ Trainer combo (reputable + hot form): +{_combo_pts}pts")
    else:
        breakdown['trainer_combo'] = 0

    # ── MARKET STEAM / DRIFT DETECTION (2026-04-20) ───────────────────────────
    # betfair_odds_fetcher.py tracks price_history.json and flags runners as
    # 'steaming' (price shortened ≥20%) or 'drifting' (lengthened ≥25%).
    # Steaming = smart money coming in (very strong positive signal).
    # Drifting = market losing confidence (negative but not fatal).
    _price_move = str(horse_data.get('price_movement', '')).lower()
    _price_pct = abs(int(horse_data.get('price_move_pct', 0) or 0))
    if _price_move == 'steaming':
        _steam_pts = int(weights.get('market_steam_bonus', 10))
        score += _steam_pts
        breakdown['market_steam'] = _steam_pts
        reasons.append(f"🔥 Market steam: price shortened {_price_pct}% (money coming in): +{_steam_pts}pts")
    elif _price_move == 'drifting':
        _drift_pts = int(weights.get('market_drift_penalty', 6))
        score -= _drift_pts
        breakdown['market_steam'] = -_drift_pts
        reasons.append(f"⚠️ Market drift: price lengthened {_price_pct}% (confidence dropping): -{_drift_pts}pts")
    else:
        breakdown['market_steam'] = 0

    # ── TIMEFORM STAR RATING (2026-04-20) ─────────────────────────────────────
    # Sporting Life exposes Timeform star ratings (1-5) for free on racecards.
    # 5 stars = Timeform's top pick in race. Very strong positive signal.
    # 4 stars = highly rated. 3 = average. 1-2 = Timeform sees poor chance.
    _tf_stars = horse_data.get('timeform_stars')
    if _tf_stars and isinstance(_tf_stars, (int, float)):
        _tf_stars = int(_tf_stars)
        if _tf_stars >= 5:
            _tf_pts = int(weights.get('timeform_5star_bonus', 12))
            score += _tf_pts
            breakdown['timeform_stars'] = _tf_pts
            reasons.append(f"⭐ Timeform 5-star rating (top pick): +{_tf_pts}pts")
        elif _tf_stars == 4:
            _tf_pts = int(weights.get('timeform_4star_bonus', 8))
            score += _tf_pts
            breakdown['timeform_stars'] = _tf_pts
            reasons.append(f"⭐ Timeform 4-star rating (well rated): +{_tf_pts}pts")
        elif _tf_stars == 3:
            _tf_pts = int(weights.get('timeform_3star_bonus', 4))
            score += _tf_pts
            breakdown['timeform_stars'] = _tf_pts
            reasons.append(f"Timeform 3-star rating (average): +{_tf_pts}pts")
        elif _tf_stars <= 2:
            _tf_pen = int(weights.get('timeform_lowstar_penalty', 6))
            score -= _tf_pen
            breakdown['timeform_stars'] = -_tf_pen
            reasons.append(f"Timeform {_tf_stars}-star rating (poor view): -{_tf_pen}pts")
    else:
    ...
    # PHASE 1 FREE SIGNALS (2026-05-20)
    # ══════════════════════════════════════════════════════════════════════════
    # Quick integration: Run Style + Jockey Upgrade (use existing data)
    # Equipment + Liquidity pending data extraction updates

    if PHASE1_SIGNALS_AVAILABLE:

        # ── RUN STYLE & PACE MATCH (Signal #1) ────────────────────────────────
        # Classify horse's running style and match to predicted race pace
        form_runs = horse_data.get('form_runs', [])
        run_style = horse_data.get('run_style', 'UNKNOWN')

        # If not already classified, do it now
        if run_style == 'UNKNOWN' and form_runs:
            run_style = classify_run_style(form_runs, horse_data.get('form', ''))
            horse_data['run_style'] = run_style  # Cache for future use

        # Get predicted race pace (set by get_comprehensive_pick before scoring)
        predicted_pace = horse_data.get('_race_predicted_pace', 'STEADY')

        # Calculate pace match bonus/penalty
        pace_pts, pace_reason = calculate_pace_match_bonus(run_style, predicted_pace, weights)
        if pace_pts != 0:
            score += pace_pts
            breakdown['pace_match'] = pace_pts
            if pace_reason:
                reasons.append(f"[PHASE1] {pace_reason}")
        else:
            breakdown['pace_match'] = 0

        # ── JOCKEY UPGRADE (Signal #3) ────────────────────────────────────────
        # Detect elite jockey booking vs recent runs (stable confidence)
        current_jockey = horse_data.get('jockey', '')

        if current_jockey and form_runs:
            # Check for upgrade
            upgrade_pts, upgrade_reason = detect_jockey_upgrade(current_jockey, form_runs, weights)

            # Check for downgrade
            if upgrade_pts == 0:
                downgrade_pts, downgrade_reason = detect_jockey_downgrade(current_jockey, form_runs, weights)
                if downgrade_pts != 0:
                    score += downgrade_pts  # Already negative
                    breakdown['jockey_upgrade'] = downgrade_pts
                    if downgrade_reason:
                        reasons.append(f"[PHASE1] {downgrade_reason}")
                else:
                    breakdown['jockey_upgrade'] = 0
            else:
                score += upgrade_pts
                breakdown['jockey_upgrade'] = upgrade_pts
                if upgrade_reason:
                    reasons.append(f"[PHASE1] {upgrade_reason}")
        else:
            breakdown['jockey_upgrade'] = 0

        # ── EQUIPMENT CHANGES (Signal #2) ─────────────────────────────────────
        # PENDING: Requires Sporting Life HTML extraction update
        # Will be activated when form_enricher extracts equipment field
        equipment_bonus = horse_data.get('_phase1_equipment_bonus', 0)
        if equipment_bonus > 0:
            score += equipment_bonus
            breakdown['first_time_equipment'] = equipment_bonus
            equipment_reasons = horse_data.get('_phase1_equipment_reasons', [])
            reasons.extend([f"[PHASE1] {r}" for r in equipment_reasons])
        else:
            breakdown['first_time_equipment'] = 0

        # ── MARKET LIQUIDITY (Signal #4) ──────────────────────────────────────
        # PENDING: Requires Betfair matched_volume extraction
        # Will be activated when betfair_fetcher extracts matched volume
        liquidity_bonus = horse_data.get('_phase1_liquidity_bonus', 0)
        if liquidity_bonus != 0:
            score += liquidity_bonus
            breakdown['market_liquidity'] = liquidity_bonus
            liquidity_reason = horse_data.get('_phase1_liquidity_reason', '')
            if liquidity_reason:
                reasons.append(f"[PHASE1] {liquidity_reason}")
        else:
...
def get_comprehensive_pick(race_data, course_stats=None, meeting_context=None):
    ...
    # ─── PHASE 1: ENRICH RUNNERS BEFORE SCORING ───────────────────────────────
    # Classify run styles and predict race pace BEFORE individual horse scoring
    if PHASE1_SIGNALS_AVAILABLE and runners:
        # Classify each runner's style (uses form_runs if available)
        for runner in runners:
            form_runs = runner.get('form_runs', [])
            if form_runs:
                run_style = classify_run_style(form_runs, runner.get('form', ''))
                runner['run_style'] = run_style
            else:
                runner['run_style'] = 'UNKNOWN'

        # Predict race pace based on all runners' styles
        predicted_pace = predict_race_pace(runners)

        # Inject predicted pace into each runner for use in scoring
        for runner in runners:
            runner['_race_predicted_pace'] = predicted_pace

        print(f"[PHASE1] Race pace predicted: {predicted_pace} | Run styles classified: {len(runners)} runners")
    ...
    # 2026-04-24 guardrails: model-vs-market disagreement controls
    # 1) Market divergence: if top model pick has materially bigger odds than #2 with only a small
    #    score edge, reduce top score to avoid forcing weak rank flips against market evidence.
    # 2) Score-gap illusion: if model shows a huge score gap but market prices top two near-equal,
    #    reduce top score (signals likely factor overfit in this race).
    if len(analyzed_horses) >= 2:
        top = analyzed_horses[0]
        second = analyzed_horses[1]
        top_odds = float((top.get('horse') or {}).get('odds') or 0)
        second_odds = float((second.get('horse') or {}).get('odds') or 0)
        top_score = float(top.get('score') or 0)
        second_score = float(second.get('score') or 0)
        score_gap = top_score - second_score

        md_pen = int(get_dynamic_weights().get('market_divergence_penalty', 14))
        sgi_pen = int(get_dynamic_weights().get('score_gap_illusion_penalty', 12))

        applied_penalty = 0
        applied_reason = None

        # Market strongly prefers #2 but model barely separates them.
        if top_odds > 0 and second_odds > 0 and top_odds >= (1.5 * second_odds) and score_gap <= 20:
            applied_penalty = md_pen
            applied_reason = (
                f"Model/market divergence: top pick odds {top_odds:.2f} vs #2 {second_odds:.2f} "
                f"with only {score_gap:.0f}pts score gap"
            )
            top['breakdown']['market_divergence_penalty'] = -applied_penalty

        # Very large model gap but market prices are near-equal -> overconfidence warning.
        elif top_odds > 0 and second_odds > 0 and score_gap >= 40 and abs(top_odds - second_odds) <= 0.5:
            applied_penalty = sgi_pen
            applied_reason = (
                f"Score-gap illusion: {score_gap:.0f}pts model gap but market odds near-equal "
                f"({top_odds:.2f} vs {second_odds:.2f})"
            )
            top['breakdown']['score_gap_illusion_penalty'] = -applied_penalty

        if applied_penalty > 0:
            top['score'] = max(0, top_score - applied_penalty)
            top['reasons'].append(f"⚠️ {applied_reason}: -{applied_penalty}pts")
            analyzed_horses.sort(key=lambda x: x['score'], reverse=True)
...
def format_pick_for_database(pick_data, race_data):
    ...
    # Persist picks against the race day (from race_time), not processing day.
    # This keeps reruns idempotent and avoids duplicate rows across adjacent days.
    bet_date = datetime.now(timezone.utc).strftime('%Y-%m-%d')
    try:
        if race_time:
            bet_date = datetime.fromisoformat(str(race_time).replace('Z', '+00:00')).strftime('%Y-%m-%d')
    except Exception:
        pass
    ...
    # Latest settled audit (2026-05-12): Empirical gate optimization
    # - Confidence < 75: 7.55% SR (BROKEN) — remove entirely
    # - Confidence 75-85: 27.5% SR (PROVEN) — core sweet spot
    # - Odds 3-8: 13-17% SR (WEAK) — gate out unless confidence 85+
    # - Odds <2 or >8: 26-40% SR (STRONG) — keep
    ...
    _confidence       = float(score)  # confidence_level is represented by score
    mid_odds_band     = 3.0 <= _pick_odds < 5.0
    weak_live_band    = 5.0 <= _pick_odds <= 8.0
    inflated_score    = score >= 130
    weak_odds_band    = 3.0 <= _pick_odds <= 8.0  # 13-17% SR empirically weak
    # ATOMIC FIX #1: Raise confidence floor from 65 to 75 (removes 7.55% SR band)
    confidence_too_low = _confidence < 75
    # ATOMIC FIX #2: Gate weak odds band unless high confidence
    weak_odds_needs_high_conf = weak_odds_band and _confidence < 85
    show_on_ui        = (score >= 92) and not aw_penalised and not extreme_odds and not low_coverage and not confidence_too_low and not weak_odds_needs_high_conf
    if mid_odds_band and score < 98:
        show_on_ui = False
    if weak_live_band and score < 110:
        show_on_ui = False
    if inflated_score and _pick_odds >= 3.0 and score < 145:
        show_on_ui = False
    recommended_bet = show_on_ui
    # Add gates to pick metadata so UI can show why something was filtered
    if confidence_too_low:   pick_data['reasons'].append(f'FILTERED: Confidence {_confidence:.0f} < 75 (empirically weak band)')
    if weak_odds_needs_high_conf:   pick_data['reasons'].append(f'FILTERED: Weak odds band {_pick_odds:.1f} requires confidence 85+; scored {_confidence:.0f}')
    ...
    if mid_odds_band and score < 98:
        pick_data['reasons'].append(f'FILTERED: Odds {_pick_odds:.1f} in 3-5 band; need 98+ score to show')
    if weak_live_band and score < 110:
        pick_data['reasons'].append(f'FILTERED: Odds {_pick_odds:.1f} in 5-8 band; weak live SR requires 110+ score')
    if inflated_score and _pick_odds >= 3.0 and score < 145:
        pick_data['reasons'].append(f'FILTERED: Score {score} is in inflated 130+ band; need 145+ outside short-odds range')
