...
try:
    from post_race_analyser import persist_post_race_analysis as _auto_analyse
    _POST_RACE_AVAILABLE = True
except ImportError:
    _POST_RACE_AVAILABLE = False
...
def update_bet_with_result(bet, market_data):
    ...
    try:
        ...
        # Auto-generate post-race analysis answers for all 12 questions
        if _POST_RACE_AVAILABLE:
            # Merge result data into the bet record so the analyser has full context
            enriched = dict(bet)
            enriched['outcome'] = outcome
            enriched['actual_result'] = actual_result
            enriched['race_winner_name'] = winner_name
            enriched['winner_name'] = winner_name
            enriched['placed_horses'] = placed_horses
            if _auto_analyse(table, enriched):
                print(f"  📋 Post-race analysis written for {bet['horse']}")
